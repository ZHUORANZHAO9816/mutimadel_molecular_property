...
from ..model.models import GROVEREmbedding, GroverFpGeneration, GroverFinetuneTask
...
def _load_checkpoint_state(path: str):
    """Load a legacy GROVER checkpoint on CPU across PyTorch versions.

    GROVER checkpoints include an ``argparse.Namespace`` alongside tensor
    weights. Modern PyTorch defaults to restricted weights-only loading, so the
    known metadata class is explicitly allowlisted. Older supported PyTorch
    releases do not expose ``safe_globals``/``weights_only`` and retain their
...
def load_checkpoint(path: str,
    ...
    for param_name in loaded_state_dict.keys():
        # Official GROVER pretraining checkpoints store encoder weights below a
        # ``grover.`` prefix, while a directly saved GROVEREmbedding state dict
        # has no prefix. Support both explicit schemas.
        new_param_name = param_name[7:] if param_name.startswith('grover.') else param_name
    ...
    if not pretrained_state_dict:
        raise RuntimeError(
            f'Checkpoint {path!r} contains no parameters compatible with GROVEREmbedding.'
        )
    debug(
        f'Loaded {len(pretrained_state_dict)}/{len(model_state_dict)} '
        'GROVEREmbedding parameter tensors.'
    )
