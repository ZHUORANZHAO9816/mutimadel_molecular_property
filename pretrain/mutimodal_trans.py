...
from gtpro.models.interfaces import AlignmentLosses
...
class CoCa(nn.Module):
    def __init__(
        ...
        *,
        ...
        functional_group_loss_weight=1.,
        ...
        image_dim = default(image_dim, dim)

        self.pad_id = pad_id
        self.caption_loss_weight = caption_loss_weight
        self.contrastive_loss_weight = contrastive_loss_weight
        self.functional_group_loss_weight = functional_group_loss_weight
        ...
        self.to_dim = nn.Sequential(
            LayerNorm(image_dim),
            nn.Linear(image_dim, dim, bias=True)
        )
    ...
    def forward(
        ...
        if batch == 0:
            raise ValueError("CoCa received an empty batch")
        ...
        seq = text_all.shape[1]
        for attn_ff, cross_attn in self.multimodal_layers:
            text_tokens = attn_ff(text_all)
            if self.use_cross_attention:
                text_tokens = text_tokens.reshape([batch * seq, self.dim])
        ...
        subgraph = self.sub_graph(text_tokens[:, :1]).squeeze(1)
        logits = logits[:, 1:]
        ...
        subgraph_loss = fg_task_loss(subgraph, fg_feature) * self.functional_group_loss_weight

        logits = logits.reshape([batch * (seq - 1), self.num_tokens])
        labels = labels.reshape([batch * (seq - 1), self.num_tokens])
        atom_mask = atom_mask.reshape(batch * (seq - 1), 1).bool()
        atom_loss_values = loss_criterion_atom(logits, labels)
        supervised_values = atom_mask.sum() * self.num_tokens
        if supervised_values.item() == 0:
            caption_loss = atom_loss_values.sum() * 0.0
        else:
            caption_loss = (atom_loss_values * atom_mask).sum() / supervised_values
        ...
        total = caption_loss + subgraph_loss + contrastive_loss
        return AlignmentLosses(
            atom=caption_loss,
            functional_group=subgraph_loss,
