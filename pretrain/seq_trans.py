...
from gtpro.models.interfaces import TextEncoderOutput, encode_graph
from descriptastorus.descriptors import rdDescriptors
import os


def get_attn_pad_mask(seq_q):
    """Return ``[batch, query, key]`` mask where True keys are padding."""

    batch_size, seq_len = seq_q.size()
    pad_attn_mask = seq_q.eq(0).unsqueeze(1)
...
class Embedding(nn.Module):
    ...
    def forward(self, x):
        ...
        pos = pos.unsqueeze(0).expand_as(x).to(x.device)
...
class K_BERT_WCL(nn.Module):
    ...
    def forward(self, input_ids):
        ...
        return TextEncoderOutput(
            ...
            atom_tokens=h_atom,
            molecule_logits=self.classifier_global(h_global),
        )
...
class EarlyStopping(object):
    def __init__(self, pretrained_model='Null_early_stop.pth',
        ...
        self.counter = 0
        self.filename = f'../model/{task_name}_early_stop.pth'
        self.pretrain_save_filename = f'../model/pretrain_{task_name}_epoch_'
        self.best_score = None
        self.early_stop = False
        self.pretrained_model = f'../model/{pretrained_model}'
    ...
    def pretrain_step(self, epoch, model):
        print(f'Pretrain epoch {epoch} is finished and the model is saved')
...
def run_a_pretrain_epoch(args, args_g, epoch, model_bert, model_grover, model_coca, data_loader,
    ...
    for batch_id, batch_data in enumerate(data_loader):
        ...
        graph_output = encode_graph(model_grover, batchgraph, device)
        ...
        #grover
        token_idx = token_idx.long().to(device)
        if atom_labels.shape[1] == token_idx.shape[1]:
            atom_labels = atom_labels[:, 1:]
            atom_mask = atom_mask[:, 1:]
        atom_labels = atom_labels.float().to(device)
        atom_mask = atom_mask.float().to(device)
        fg_feature = fg_feature.float().to(device)
        text_output = model_bert(token_idx)
        ...
        print(f'epoch {epoch + 1}/{args["num_epochs"]}, batch {batch_id + 1}/{len(data_loader)}, loss {loss:.4f}')
        del token_idx, global_labels, atom_labels, atom_mask, loss, text_output, batchgraph, graph_output
    print(f'epoch {epoch + 1}/{args["num_epochs"]}, pre-train loss {total_loss:.4f}')
    torch.save(model_coca.state_dict(), os.path.join(save_dir, f'model_coca{epoch}.pth'))
    torch.save(model_bert.state_dict(), os.path.join(save_dir, f'model_bert{epoch}.pth'))
    return total_loss
